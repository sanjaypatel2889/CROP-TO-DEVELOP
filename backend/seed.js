// Seed script for deployment (runs inside backend/ directory)
...
function seed() {
  ...
  if (cropCount > 0) {
    console.log(`[Seed] Database already has ${cropCount} crops. Skipping seed.`);
    return;
  }
  ...
  const seedAll = db.transaction(() => {
      ...
      diseases.forEach(d => {
        try {
          ...
          insertDisease.run(
            ...
            JSON.stringify(d.symptoms || []), JSON.stringify(d.leaf_indicators || {}),
            JSON.stringify(d.severity_levels || []), JSON.stringify(d.favorable_conditions || {}),
            JSON.stringify(d.treatment || []), JSON.stringify(d.prevention || []),
      ...
      pests.forEach(p => {
        try {
          insertPest.run(
            ...
            JSON.stringify(p.target_crops || []), JSON.stringify(p.damage_symptoms || []),
            p.lifecycle_days || null, p.active_season || null, JSON.stringify(p.active_months || []),
            p.favorable_temp_min, p.favorable_temp_max,
            p.favorable_humidity_min, p.favorable_humidity_max,
            JSON.stringify(p.risk_factors || {}),
            JSON.stringify(p.control_chemical || []), JSON.stringify(p.control_biological || []),
            JSON.stringify(p.control_cultural || []), p.severity_if_untreated || 'medium'
          );
        } catch (e) { /* skip */ }
      });
    ...
    if (soilTypes.length) {
      ...
      soilTypes.forEach(s => {
        try {
          insertSoil.run(
            s.name, s.name_hindi, s.description, JSON.stringify(s.regions || []),
            JSON.stringify(s.characteristics || {}), JSON.stringify(s.nutrient_profile || {}),
            JSON.stringify(s.suitable_crops || []), JSON.stringify(s.recommended_fertilizers || []),
      ...
      schemes.forEach(s => {
        try {
          insertScheme.run(
            ...
            s.benefit_amount, JSON.stringify(s.eligibility_criteria || {}),
            JSON.stringify(s.required_documents || []), JSON.stringify(s.application_process || {}),
            s.application_url || null, s.helpline || null,
            s.is_active !== false ? 1 : 0, s.valid_from || null, s.valid_to || null,
    ...
    if (marketPrices.length) {
      ...
      marketPrices.forEach(mp => {
        try {
          ...
          if (crop) {
            insertPrice.run(crop.id, mp.mandi_name, mp.state, mp.district || null,
              mp.price_per_quintal, mp.price_date, mp.price_type || 'modal', mp.arrival_tonnes || null);
            inserted++;
          }
        } catch (e) { /* skip */ }
      });
      ...
      const aggregated = db.prepare(`
        SELECT crop_id, state, CAST(strftime('%m', price_date) AS INTEGER) as month,
          CAST(strftime('%Y', price_date) AS INTEGER) as year,
          ROUND(AVG(price_per_quintal), 2) as avg_price, MIN(price_per_quintal) as min_price,
          MAX(price_per_quintal) as max_price
        FROM market_prices GROUP BY crop_id, state, year, month ORDER BY crop_id, state, year, month
      `).all();
      let histCount = 0;
      for (let i = 0; i < aggregated.length; i++) {
        ...
        if (i > 0 && aggregated[i - 1].crop_id === row.crop_id && aggregated[i - 1].state === row.state) {
          const change = (row.avg_price - aggregated[i - 1].avg_price) / aggregated[i - 1].avg_price;
          if (change > 0.03) trend = 'rising';
          else if (change < -0.03) trend = 'falling';
        }
        try { insertHistory.run(row.crop_id, row.state, row.month, row.year, row.avg_price, row.min_price, row.max_price, trend); histCount++; } catch (e) { /* skip */ }
      }
      console.log(`  Generated ${histCount} price history records`);
    }
    ...
    if (farmers.length) {
      ...
      farmers.forEach(f => {
        try {
          insertFarmer.run(f.name, f.phone, f.state, f.district, f.village || null,
            f.land_size_hectares || 1.0, f.soil_type || 'alluvial', f.primary_crop || null,
            f.irrigation_type || 'rainfed', f.income_category || 'small');
        } catch (e) { /* skip */ }
      });
